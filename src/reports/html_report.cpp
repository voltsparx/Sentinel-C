...
std::string write_html(const scanner::ScanResult& result, const std::string& scan_id) {
    ...
    std::vector<std::string> guidance_lines;
    guidance_lines.reserve(narrative.whys.size() +
                           narrative.what_matters.size() +
                           narrative.teaching.size() +
                           narrative.next_steps.size());
    guidance_lines.insert(guidance_lines.end(), narrative.whys.begin(), narrative.whys.end());
    guidance_lines.insert(guidance_lines.end(), narrative.what_matters.begin(), narrative.what_matters.end());
    guidance_lines.insert(guidance_lines.end(), narrative.teaching.begin(), narrative.teaching.end());
    guidance_lines.insert(guidance_lines.end(), narrative.next_steps.begin(), narrative.next_steps.end());

    out << "      <div class='advisor-grid'>\n";
    write_advisor_list(out, "Guidance", guidance_lines,
                       "No extra guidance was needed for this scan.");
    out << "      </div>\n";
    out << "    </section>\n";
